export class TicketmasterService {
    ...
    openai; // Will be set from constructor
    lastRequestTime = 0;
    requestDelay = 200; // 200ms between requests to respect rate limits
    constructor(apiKey, openai) {
        this.apiKey = apiKey;
        this.openai = openai;
    }
    async searchEventsByCity(city, limit = 10, startDate, endDate) {
        try {
    ...
    async searchWellnessEventsByCity(city, limit = 10, startDate, endDate) {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/events.json?city=${encodeURIComponent(city)}&classificationId=${classificationQuery}&size=${limit}&apikey=${this.apiKey}`, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
    ...
    async searchConcertsByCity(city, limit = 10, startDate, endDate) {
        try {
    ...
    async searchArtistShows(artistName, limit = 10, startDate, endDate) {
        try {
            // Build the URL with proper parameters according to Ticketmaster API docs
            let url = `${this.baseUrl}/events.json?apikey=${this.apiKey}&size=${limit}`;
    ...
    async searchArtistShowsInCity(artistName, city, limit = 10, startDate, endDate) {
        try {
            // Build the URL with proper parameters according to Ticketmaster API docs
            let url = `${this.baseUrl}/events.json?apikey=${this.apiKey}&size=${limit}`;
    ...
    async searchAttractionByName(artistName) {
        try {
            ...
            if (attractions.length > 0) {
                ...
                const exactMatch = attractions.find((attraction) => attraction.name.toLowerCase() === artistName.toLowerCase());
                if (exactMatch) {
    ...
    async searchEventsByAttractionId(attractionId, city, limit = 10, startDate, endDate) {
        try {
            let url = `${this.baseUrl}/events.json?apikey=${this.apiKey}&size=${limit}`;
    ...
    async searchEventsByGenre(city, genre, limit = 10, startDate, endDate) {
        try {
            // Build the URL with proper parameters according to Ticketmaster API docs
            let url = `${this.baseUrl}/events.json?apikey=${this.apiKey}&size=${limit}`;
            // Add city parameter
     ...
     */
    async parseSearchQuery(query, userCity) {
        if (!this.openai) {
...
Return ONLY the JSON object, no markdown formatting, no code blocks, no additional text.`;
    ...
    async searchWithFlexibleLocation(searchParams, startDate, endDate) {
     ...
     */
    basicParseSearchQuery(query, userCity) {
...
Want more details on any of these? Just ask!`;
    ...
    isWellnessEvent(event) {
        ...
        return wellnessKeywords.some(keyword => eventName.includes(keyword) || segmentName.includes(keyword));
    }
