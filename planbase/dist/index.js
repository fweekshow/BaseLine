...
import { WellnessRouter } from './router.js';
...
const { WALLET_KEY, ENCRYPTION_KEY, OPENAI_API_KEY, TICKETMASTER_API_KEY, XMTP_ENV } = validateEnvironment([
    "WALLET_KEY",
    "ENCRYPTION_KEY",
    "OPENAI_API_KEY",
...
const pendingAddressRequests = new Set();
/**
        ...
        try {
            ...
            for await (const message of await stream) {
                ...
                try {
                    ...
                    const isMentioned = isGroup && (messageContent.toLowerCase().includes('@wellness') ||
...
To remove it, type "Remove my address"`);
                    ...
                    if (!isGroup && (messageContent.toLowerCase().includes('i live in') ||
...
${eventsData}`;
                    ...
                    try {
                        await conversation.send("Sorry, I encountered an error. Here are the two options I can share right now:\n\n" + getCombinedResponse());
                    }
                    catch (sendError) {
